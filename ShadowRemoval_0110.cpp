...
void main(){
	...
	/*	メモリ(画素数分)の確保	*/
	prImage = (LIGHTING_UINT8*)malloc(sizeof(LIGHTING_UINT8)*IMG_WIDTH*IMG_HEIGHT);
	pwImage = (LIGHTING_UINT8*)malloc(sizeof(LIGHTING_UINT8)*IMG_WIDTH*IMG_HEIGHT);

	/*	メモリ(画素数分)の初期化	*/
	memset(prImage, 0, sizeof(unsigned char)*IMG_WIDTH*IMG_HEIGHT);
	memset(pwImage, 0, sizeof(unsigned char)*IMG_WIDTH*IMG_HEIGHT);

	sprintf(frname, "road01.pgm");
	read_pgm_file(prImage, IMG_WIDTH, IMG_HEIGHT, frname);

	retVal = gauSmooth(prImage, pwImage);
	retVal = geoLevel(pwImage, pwImage);
	...
	write_pgm_file(pwImage, IMG_WIDTH, IMG_HEIGHT, fwname);
	...
	/************************************************/
{
	...
	double Is = 0, Ib = 0, Ds = 0, Db = 0, alpha, lambda, devian, ave;
...
//					fprintf(fp, "Geo_level:%3d, size:%4d, x:%3d, y:%3d, val:%3d\n", Geo_Level, Gi->NUMBER[Geo_Level].GiPoint.size(), Gi->NUMBER[Geo_Level].GiPoint[level_sum].x, Gi->NUMBER[Geo_Level].GiPoint[level_sum].y, Gi->NUMBER[Geo_Level].GiPoint[level_sum].val);
		...
		if(level_sum >= Ng){
			...
			for(level_sum2=0; level_sum2<level_sum; level_sum2++){
				devian+= pow(Gi->NUMBER[Geo_Level].GiPoint[level_sum2].val-ave, 2);
			}
			devian /= level_sum;
			fprintf(fp, "%d %lf\n", Geo_Level, sqrt(devian));
#endif
			level_sum = 0;
			Geo_Level++;
		}
