...
void main(){
	...
	time(&Time);
	Now = localtime(&Time);
	sprintf(fname, "..\\Result\\%4d_%2d_%2d_%2d_%2d_%2d_geoLevel_%d.pgm", 1900+Now->tm_year, Now->tm_mon+1, Now->tm_mday, Now->tm_hour, Now->tm_min, Now->tm_sec, N);
	...
	/************************************************/
{
		...
		for(y=0; y<IMG_HEIGHT; y++){
			for(x=0; x<IMG_WIDTH; x++){
				if(pInImg[y*IMG_WIDTH+x] == bright){
					point.x = x;
					point.y = y;
					Gi->NUMBER[Geo_Level].Gi.push_back(point);
					Gi->NUMBER[Geo_Level].val = pInImg[y*IMG_WIDTH+x];
					pOutImg[y*IMG_WIDTH+x] = Geo_Level;
					fprintf(fp, "Geo_level:%3d, number:%4d, size:%4d, x:%3d, y:%3d, val:%3d\n", Geo_Level, level_sum, Gi->NUMBER[Geo_Level].Gi.size(), Gi->NUMBER[Geo_Level].Gi[level_sum].x, Gi->NUMBER[Geo_Level].Gi[level_sum].y, Gi->NUMBER[Geo_Level].val);
	...
	*N = Geo_Level;
	...
	for(Geo_Level=0; Geo_Level<=*N; Geo_Level++){
		...
		for(count=0; count<level_sum; count++){
			if(Geo_Level<L){
				...
				ave1 += pInImg[Gi->NUMBER[Geo_Level].Gi[count].y*IMG_WIDTH+Gi->NUMBER[Geo_Level].Gi[count].x];
				shadow_cnt++;
			}
			else{
				Gi->NUMBER[Geo_Level].isShadow = 0;
				ave2 += pInImg[Gi->NUMBER[Geo_Level].Gi[count].y*IMG_WIDTH+Gi->NUMBER[Geo_Level].Gi[count].x];
				nshadow_cnt++;
			}
			fprintf(fp, "Geo_level:%3d, number:%4d, x:%3d, y:%3d, val:%3d, isShadow:%s\n", Geo_Level, count, Gi->NUMBER[Geo_Level].Gi[count].x, Gi->NUMBER[Geo_Level].Gi[count].y, Gi->NUMBER[Geo_Level].val, (Gi->NUMBER[Geo_Level].isShadow==1)? "TRUE" : "FALSE");
		}
	}
	ave1/=shadow_cnt;
	ave2/=nshadow_cnt;
	...
	for(Geo_Level=0; Geo_Level<=*N; Geo_Level++){
		...
		for(count=0; count<level_sum; count++){
			if(Geo_Level<L){
				var1 += pow(pInImg[Gi->NUMBER[Geo_Level].Gi[count].y*IMG_WIDTH+Gi->NUMBER[Geo_Level].Gi[count].x]-ave1, 2);
			}
			else{
				var2 += pow(pInImg[Gi->NUMBER[Geo_Level].Gi[count].y*IMG_WIDTH+Gi->NUMBER[Geo_Level].Gi[count].x]-ave2, 2);
			}
		}
	}
	var1/=shadow_cnt;
	...
	for(Geo_Level=0; Geo_Level<=*N; Geo_Level++){
		...
		for(count=0; count<level_sum; count++){
			if(Geo_Level<L){
				pOutImg[Gi->NUMBER[Geo_Level].Gi[count].y*IMG_WIDTH+Gi->NUMBER[Geo_Level].Gi[count].x] = alpha*pInImg[Gi->NUMBER[Geo_Level].Gi[count].y*IMG_WIDTH+Gi->NUMBER[Geo_Level].Gi[count].x]/*+lambda*/;
			}
			else{
				pOutImg[Gi->NUMBER[Geo_Level].Gi[count].y*IMG_WIDTH+Gi->NUMBER[Geo_Level].Gi[count].x] = pInImg[Gi->NUMBER[Geo_Level].Gi[count].y*IMG_WIDTH+Gi->NUMBER[Geo_Level].Gi[count].x];
			}
		}
	}
