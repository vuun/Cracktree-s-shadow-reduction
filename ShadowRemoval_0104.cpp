...
void main(){
	...
	time(&Time);
	Now = localtime(&Time);
	sprintf(fname, "C:\\temp\\result\\%d%d%d_%dh%dm%ds_geoLevel_%d.pgm", 1900+Now->tm_year, Now->tm_mon+1, Now->tm_mday, Now->tm_hour, Now->tm_min, Now->tm_sec, N);
	...
	/************************************************/
{
	...
	LIGHTING_INT32 bright, x, y, count, L;
	...
	*N = Geo_Level;
	...
	for(Geo_Level=0; Geo_Level<=Ng; Geo_Level++){
		for(count=0; count<Gi->NUMBER[Geo_Level].sum; count++){
			fprintf(fp, "Geo_level:%3d, sum:%4d, number:%4d, x:%3d, y:%3d, val:%3d\n", Geo_Level, Gi->NUMBER[Geo_Level].sum, count, Gi->NUMBER[Geo_Level].point[count].x, Gi->NUMBER[Geo_Level].point[count].y, Gi->NUMBER[Geo_Level].val);
		}
	}
